export const translations = {
  zh: {
    'brand.logo': '栖居 QIJU',
    'brand.footer': '栖居 QIJU',
    'brand.desc': '为现代都市生活打造的简约家具系列。<br>北欧设计 · 实木工艺 · 适度生活',
    'nav.storage': '储物',
    'nav.sleep': '睡眠',
    'nav.dining': '用餐',
    'nav.work': '工作',
    'nav.living': '客厅',
    'hero.label': '春季新品',
    'hero.title': '让家更有温度',
    'hero.subtitle': '北欧极简设计，实木 craftsmanship，为现代都市生活打造的简约家具系列',
    'btn.browse': '浏览系列',
    'btn.learnMore': '了解更多',
    'category.title': '按功能浏览',
    'category.storage': '储物',
    'category.sleep': '睡眠',
    'category.dining': '用餐',
    'category.work': '工作',
    'category.living': '客厅',
    'product.title': '本周精选',
    'product.viewAll': '查看全部 →',
    'products.empty': '暂无相关商品',
    'products.backToHome': '返回首页',
    'badge.new': '新品',
    'detail.backToProducts': '返回商品列表',
    'detail.addToCart': '加入购物车',
    'detail.buyNow': '立即购买',
    'detail.quantity': '数量',
    'detail.selectColor': '选择颜色',
    'detail.description': '商品描述',
    'detail.descriptionText': '精选实木材质，北欧简约设计，为您的家居空间增添自然温馨的氛围。',
    'detail.specifications': '规格参数',
    'detail.shipping': '配送信息',
    'detail.notFound': '商品未找到',
    'detail.backToHome': '返回首页',
    'detail.warranty': '质保信息',
    'detail.material': '材质',
    'detail.dimensions': '尺寸',
    'detail.color': '颜色',
    'footer.products': '产品',
    'footer.newArrivals': '新品上架',
    'footer.bestsellers': '热销系列',
    'footer.sale': '限时优惠',
    'footer.giftCard': '礼品卡',
    'footer.help': '帮助',
    'footer.shipping': '配送信息',
    'footer.returns': '退换政策',
    'footer.installation': '安装服务',
    'footer.faq': '常见问题',
    'footer.about': '关于',
    'footer.story': '品牌故事',
    'footer.philosophy': '设计理念',
    'footer.sustainability': '可持续发展',
    'footer.careers': '加入我们',
    'footer.contact': '联系',
    'footer.support': '在线客服',
    'footer.showroom': '预约展厅',
    'footer.b2b': '企业采购',
    'footer.partnership': '合作洽谈',
    'footer.copyright': '© 2026 栖居 QIJU. All rights reserved.',
    'footer.legal': '隐私政策 · 服务条款 · 沪ICP备xxxxxxxx号',
  },
  en: {
    'brand.logo': 'QIJU Home',
    'brand.footer': 'QIJU Home',
    'brand.desc': 'Minimalist furniture for modern urban living.<br>Nordic Design · Solid Wood Craftsmanship · Mindful Living',
    'nav.storage': 'Storage',
    'nav.sleep': 'Sleep',
    'nav.dining': 'Dining',
    'nav.work': 'Work',
    'nav.living': 'Living',
    'hero.label': 'New Collection',
    'hero.title': 'Warmth for Your Home',
    'hero.subtitle': 'Nordic minimalist design, solid wood craftsmanship, furniture collection crafted for modern urban life',
    'btn.browse': 'Shop Collection',
    'btn.learnMore': 'Learn More',
    'category.title': 'Browse by Function',
    'category.storage': 'Storage',
    'category.sleep': 'Sleep',
    'category.dining': 'Dining',
    'category.work': 'Work',
    'category.living': 'Living',
    'product.title': 'Featured This Week',
    'product.viewAll': 'View All →',
    'products.empty': 'No products found',
    'products.backToHome': 'Back to Home',
    'badge.new': 'New',
    'detail.backToProducts': 'Back to Products',
    'detail.addToCart': 'Add to Cart',
    'detail.buyNow': 'Buy Now',
    'detail.quantity': 'Quantity',
    'detail.selectColor': 'Select Color',
    'detail.description': 'Description',
    'detail.descriptionText': 'Crafted from solid wood with Nordic minimalist design, adding natural warmth to your living space.',
    'detail.specifications': 'Specifications',
    'detail.shipping': 'Shipping',
    'detail.notFound': 'Product Not Found',
    'detail.backToHome': 'Back to Home',
    'detail.warranty': 'Warranty',
    'detail.material': 'Material',
    'detail.dimensions': 'Dimensions',
    'detail.color': 'Color',
    'footer.products': 'Products',
    'footer.newArrivals': 'New Arrivals',
    'footer.bestsellers': 'Bestsellers',
    'footer.sale': 'Sale',
    'footer.giftCard': 'Gift Card',
    'footer.help': 'Help',
    'footer.shipping': 'Shipping',
    'footer.returns': 'Returns',
    'footer.installation': 'Installation',
    'footer.faq': 'FAQ',
    'footer.about': 'About',
    'footer.story': 'Our Story',
    'footer.philosophy': 'Philosophy',
    'footer.sustainability': 'Sustainability',
    'footer.careers': 'Careers',
    'footer.contact': 'Contact',
    'footer.support': 'Support',
    'footer.showroom': 'Showroom',
    'footer.b2b': 'B2B',
    'footer.partnership': 'Partnership',
    'footer.copyright': '© 2026 QIJU Home. All rights reserved.',
    'footer.legal': 'Privacy Policy · Terms of Service',
  },
  ja: {
    'brand.logo': '栖居 QIJU',
    'brand.footer': '栖居 QIJU',
    'brand.desc': '現代都市生活のためのミニマリスト家具。<br>北欧デザイン · 堅木の職人技 · シンプルライフ',
    'nav.storage': '収納',
    'nav.sleep': '睡眠',
    'nav.dining': 'ダイニング',
    'nav.work': 'ワーク',
    'nav.living': 'リビング',
    'hero.label': '春の新作',
    'hero.title': '家に温もりを',
    'hero.subtitle': '北欧ミニマルデザイン、堅木の職人技、現代都市生活のために作られたミニマリスト家具シリーズ',
    'btn.browse': 'コレクションを見る',
    'btn.learnMore': '詳しく見る',
    'category.title': '機能から探す',
    'category.storage': '収納',
    'category.sleep': '睡眠',
    'category.dining': 'ダイニング',
    'category.work': 'ワーク',
    'category.living': 'リビング',
    'product.title': '今週のおすすめ',
    'product.viewAll': 'すべて見る →',
    'products.empty': '関連商品がありません',
    'products.backToHome': 'ホームに戻る',
    'badge.new': '新作',
    'detail.backToProducts': '商品一覧に戻る',
    'detail.addToCart': 'カートに追加',
    'detail.buyNow': '今すぐ購入',
    'detail.quantity': '数量',
    'detail.selectColor': '色を選択',
    'detail.description': '商品説明',
    'detail.descriptionText': '堅木材を使用した北欧ミニマルデザインで、あなたの生活空間に自然な温もりを添えます。',
    'detail.specifications': '仕様',
    'detail.shipping': '配送情報',
    'detail.notFound': '商品が見つかりません',
    'detail.backToHome': 'ホームに戻る',
    'detail.warranty': '保証',
    'detail.material': '素材',
    'detail.dimensions': 'サイズ',
    'detail.color': 'カラー',
    'footer.products': '製品',
    'footer.newArrivals': '新着',
    'footer.bestsellers': 'ベストセラー',
    'footer.sale': 'セール',
    'footer.giftCard': 'ギフトカード',
    'footer.help': 'ヘルプ',
    'footer.shipping': '配送',
    'footer.returns': '返品・交換',
    'footer.installation': '設置サービス',
    'footer.faq': 'よくある質問',
    'footer.about': '私たちについて',
    'footer.story': 'ストーリー',
    'footer.philosophy': '哲学',
    'footer.sustainability': 'サステナビリティ',
    'footer.careers': '採用情報',
    'footer.contact': 'お問い合わせ',
    'footer.support': 'サポート',
    'footer.showroom': 'ショールーム',
    'footer.b2b': '法人向け',
    'footer.partnership': 'パートナーシップ',
    'footer.copyright': '© 2026 栖居 QIJU. All rights reserved.',
    'footer.legal': 'プライバシーポリシー · 利用規約',
  },
  ko: {
    'brand.logo': '栖居 QIJU',
    'brand.footer': '栖居 QIJU',
    'brand.desc': '현대 도시 생활을 위한 미니멀리스트 가구.<br>북유럽 디자인 · 목공 장인 정신 · 절제된 삶',
    'nav.storage': '수납',
    'nav.sleep': '수면',
    'nav.dining': '다이닝',
    'nav.work': '워크',
    'nav.living': '리빙',
    'hero.label': '봄 신상품',
    'hero.title': '집에 온기를 더하다',
    'hero.subtitle': '북유럽 미니멀 디자인, 목공 장인 정신, 현대 도시 생활을 위해 만들어진 미니멀리스트 가구 시리즈',
    'btn.browse': '컬렉션 보기',
    'btn.learnMore': '더 알아보기',
    'category.title': '용도별 보기',
    'category.storage': '수납',
    'category.sleep': '수면',
    'category.dining': '다이닝',
    'category.work': '워크',
    'category.living': '리빙',
    'product.title': '이번 주 추천',
    'product.viewAll': '전체보기 →',
    'products.empty': '관련 상품이 없습니다',
    'products.backToHome': '홈으로',
    'badge.new': '신상',
    'detail.backToProducts': '상품 목록으로',
    'detail.addToCart': '장바구니에 담기',
    'detail.buyNow': '바로 구매',
    'detail.quantity': '수량',
    'detail.selectColor': '색상 선택',
    'detail.description': '상품 설명',
    'detail.descriptionText': '고급 목재와 북유럽 미니멀 디자인으로 생활 공간에 자연스러운 온기를 더해드립니다.',
    'detail.specifications': '제품 사양',
    'detail.shipping': '배송 정보',
    'detail.notFound': '상품을 찾을 수 없습니다',
    'detail.backToHome': '홈으로',
    'detail.warranty': '보증 정보',
    'detail.material': '소재',
    'detail.dimensions': '치수',
    'detail.color': '색상',
    'footer.products': '제품',
    'footer.newArrivals': '신상품',
    'footer.bestsellers': '베스트셀러',
    'footer.sale': '세일',
    'footer.giftCard': '기프트카드',
    'footer.help': '도움말',
    'footer.shipping': '배송',
    'footer.returns': '반품/교환',
    'footer.installation': '설치 서비스',
    'footer.faq': '자주 묻는 질문',
    'footer.about': '소개',
    'footer.story': '브랜드 스토리',
    'footer.philosophy': '철학',
    'footer.sustainability': '지속 가능성',
    'footer.careers': '채용',
    'footer.contact': '연락처',
    'footer.support': '고객 지원',
    'footer.showroom': '쇼룸',
    'footer.b2b': 'B2B',
    'footer.partnership': '제휴',
    'footer.copyright': '© 2026 栖居 QIJU. All rights reserved.',
    'footer.legal': '개인정보처리방침 · 서비스약관',
  },
  'zh-TW': {
    'brand.logo': '棲居 QIJU',
    'brand.footer': '棲居 QIJU',
    'brand.desc': '為現代都市生活打造的簡約家具系列。<br>北歐設計 · 實木工藝 · 適度生活',
    'nav.storage': '收納',
    'nav.sleep': '睡眠',
    'nav.dining': '用餐',
    'nav.work': '工作',
    'nav.living': '客廳',
    'hero.label': '春季新品',
    'hero.title': '讓家更有溫度',
    'hero.subtitle': '北歐極簡設計，實木工藝，為現代都市生活打造的簡約家具系列',
    'btn.browse': '瀏覽系列',
    'btn.learnMore': '了解更多',
    'category.title': '按功能瀏覽',
    'category.storage': '收納',
    'category.sleep': '睡眠',
    'category.dining': '用餐',
    'category.work': '工作',
    'category.living': '客廳',
    'product.title': '本週精選',
    'product.viewAll': '查看全部 →',
    'products.empty': '暫無相關商品',
    'products.backToHome': '返回首頁',
    'badge.new': '新品',
    'detail.backToProducts': '返回商品列表',
    'detail.addToCart': '加入購物車',
    'detail.buyNow': '立即購買',
    'detail.quantity': '數量',
    'detail.selectColor': '選擇顏色',
    'detail.description': '商品描述',
    'detail.descriptionText': '精選實木材質，北歐簡約設計，為您的家居空間增添自然溫馨的氛圍。',
    'detail.specifications': '規格參數',
    'detail.shipping': '配送資訊',
    'detail.notFound': '商品未找到',
    'detail.backToHome': '返回首頁',
    'detail.warranty': '質保資訊',
    'detail.material': '材質',
    'detail.dimensions': '尺寸',
    'detail.color': '顏色',
    'footer.products': '產品',
    'footer.newArrivals': '新品上架',
    'footer.bestsellers': '熱銷系列',
    'footer.sale': '限時優惠',
    'footer.giftCard': '禮品卡',
    'footer.help': '幫助',
    'footer.shipping': '配送資訊',
    'footer.returns': '退換政策',
    'footer.installation': '安裝服務',
    'footer.faq': '常見問題',
    'footer.about': '關於',
    'footer.story': '品牌故事',
    'footer.philosophy': '設計理念',
    'footer.sustainability': '可持續發展',
    'footer.careers': '加入我們',
    'footer.contact': '聯繫',
    'footer.support': '線上客服',
    'footer.showroom': '預約展廳',
    'footer.b2b': '企業採購',
    'footer.partnership': '合作洽談',
    'footer.copyright': '© 2026 棲居 QIJU. All rights reserved.',
    'footer.legal': '隱私政策 · 服務條款 · 滬ICP備xxxxxxxx號',
  },
};

export const langData = {
  zh: { flag: '🇨🇳', name: '简体中文', htmlLang: 'zh-CN' },
  'zh-TW': { flag: '🇹🇼', name: '繁體中文', htmlLang: 'zh-TW' },
  en: { flag: '🇺🇸', name: 'English', htmlLang: 'en' },
  ja: { flag: '🇯🇵', name: '日本語', htmlLang: 'ja' },
  ko: { flag: '🇰🇷', name: '한국어', htmlLang: 'ko' },
};
